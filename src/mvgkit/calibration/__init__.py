...
from mvgkit.common import utils
from mvgkit.common.camera import Camera, CameraMatrix, RadialDistortionModel
from mvgkit.estimation import homography
...
class _ZhangsMethod:
    ...
    def _get_intrinsics(homographies):
        V = []
        for _, H in enumerate(homographies):
            # Eq. 8
            V.append(
                [
    ...
    def _get_extrinsics(
        ...
        for H in homographies:
            ...
            all_poses.append(utils.SE3(Rotation.from_matrix(R), t))
        return all_poses
    ...
    def _residual(
        ...
        for i in range(len(all_image_points)):
            ...
            T_CW = utils.SE3.from_rotvec_pose(all_extrinsics_array[i])
            camera = Camera(
                camera_matrix=camera_matrix,
                radial_distortion_model=radial_distortion_model,
                quat_CW=T_CW.R.as_quat(),
    ...
    def _bundle_adjustment(
        *,
        ...
        all_image_points: np.ndarray,
        object_points_W: np.ndarray,
    ) -> Tuple[CameraMatrix, RadialDistortionModel, List[utils.SE3]]:
        ...
        all_extrinsics = [
            utils.SE3.from_rotvec_pose(extrinsics) for extrinsics in all_extrinsics_array
        ]
        return (camera_matrix, radial_distortion_model, all_extrinsics)
    ...
    def _decompose_parameters(*, param_array: np.ndarray, n_points: int):
        ...
        camera_parameters_end_index = 5 + RadialDistortionModel.size
