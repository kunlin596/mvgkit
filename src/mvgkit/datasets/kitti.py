...
Reference:
    [1] https://www.mrt.kit.edu/z/publ/download/2013/GeigerAl2013IJRR.pdf
    [2] https://github.com/yanii/kitti-pcl/blob/master/KITTI_README.TXT
"""
...
from mvgkit import camera, image_processing, utils
...
class KittiDrive:
    def __init__(self, path: Path, unzip=False):
        ...
        with open(self._path / "info.txt") as f:
    ...
    def read_image(self, camera_id: int, indices: List[int], data_type: str = "sync"):
        data_id = f"image_{camera_id:02d}"
...
class KittiCalibration:
    ...
    def _process_lidar_to_cam(data):
        return KittiLiDARCalibration(
            ...
            T=utils.SE3(R=Rotation.from_matrix(data["R"].reshape(3, 3)), t=data["T"]),
