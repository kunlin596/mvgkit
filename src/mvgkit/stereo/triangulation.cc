...
Eigen::Array4f
geometricImagePointPairCorrection(const Eigen::Array2f& imagePoint_L,
                                  const Eigen::Array2f& imagePoint_R,
...
getOptimalPolynomialImagePointPair(const Eigen::Array2d& imagePoint_L,
                                   ...
                                   const Eigen::Matrix3d F_RL)
{
  ...
  Matrix<double, 6, 1> costs{
    costFn(roots[0]), costFn(roots[1]), costFn(roots[2]),
    costFn(roots[3]), costFn(roots[4]), costFn(roots[5]),
  };
  int minIndex = -1;
...
Eigen::Array4Xf
Triangulation::getGeometricImagePointPairCorrection(const Eigen::Array2Xf& imagePoints_L,
                                                    const Eigen::Array2Xf& imagePoints_R,
                                                    const Eigen::Matrix3f& F_RL)
{
  using namespace Eigen;
  ...
  for (size_t i = 0; i < numPoints; ++i) {
    corrections.col(i) =
      details::geometricImagePointPairCorrection(imagePoints_L.col(i), imagePoints_R.col(i), F_RL);
  }
  return corrections;
}

std::pair<Eigen::Array2Xd, Eigen::Array2Xd>
Triangulation::getOptimalPolynomialImagePointPairs(const Eigen::Array2Xd& imagePoints_L,
                                                   ...
                                                   const Eigen::Matrix3d F_RL)
{
  ...
  for (size_t i = 0; i < imagePoints_L.cols(); ++i) {
    auto&& [p1, p2] =
      details::getOptimalPolynomialImagePointPair(imagePoints_L.col(i), imagePoints_R.col(i), F_RL);
                                           ...
                                           const mvgkit::common::SE3f& T_RL)
{
  ...
  auto&& [optimalImagePoints_L, optimalImagePoints_R] =
    Triangulation::getOptimalPolynomialImagePointPairs(
