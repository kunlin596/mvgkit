...
TEST(MathTest, TestFindMonicPolynomialRootsTrivialCoeffs)
{
  ...
  // x^2 - 1 = 0
  auto poly1 = MonicPolynomialFunctor<2, double>({ 1.0, 0.0, -1.0 });
  ...
  // x^2 -2x + 1 = 0
  auto poly2 = MonicPolynomialFunctor<2, double>({ 1.0, -2.0, 1.0 });
  ...
  auto poly3 =
    MonicPolynomialFunctor<6, double>({ 1.0, -21.0, 175.0, -735.0, 1624.0, -1764.0, 720.0 });
  ...
  auto poly4 =
    MonicPolynomialFunctor<6, double>({ 2.0, -41.0, 330.0, -1315.0, 2668.0, -2484.0, 720.0 });
  ...
  // x^2 + 1 = 0
  auto poly6 = MonicPolynomialFunctor<2, double>({ 1.0, 0.0, 1.0 });
...
TEST(MathTest, TestRootsFindingEigenComparison)
{
  // using namespace mvgkit::common;
  // srand(42);
  // Eigen::Matrix<double, 7, 1> coeffs = Eigen::Matrix<double, 7, 1>::Random();
  // Eigen::PolynomialSolver<double, Eigen::Dynamic> solver;
  // solver.compute(coeffs);
  // auto roots2 = findMonicPolynomialRoots<6>(coeffs);
  // EXPECT_NEAR((roots1 - roots2).norm(), 0.0, 1e-6);
}
